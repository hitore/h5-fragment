!function() {
  ...
  function getDom() {
    ...
    var style = 'position: fixed; left: 50%; top: 0; transform: translate(-50%, 0); width: 100%; height: 100vh; background: rgba(0,0,0,0); max-width: 750px; z-index: 999; display: flex; align-items: center; justify-content: center;';
  ...
  function fadeAnimate(fn) {
    var target = 0;
    if (_type === 'fadeIn') target = 0.5;
    window.requestAnimationFrame(function() {
      var current = parseFloat(_loadingDom.style.background.slice(14).replace(/\)/, ''));
      var diff = target - current;
      if (_start < _end) {
        _start += 1;
        var next = SineEaseOut(_start, current, diff, _end);
        _loadingDom.style.background = 'rgba(0,0,0,'+ next +')';
        fadeAnimate(fn);
      } else {
        _start = 0;
        _loadingDom.style.background = 'rgba(0,0,0,'+ target +')';
